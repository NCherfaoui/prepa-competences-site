...
import { SiReact, SiSpring, SiPostgresql, SiMongodb, SiMysql, SiDocker, SiTailwindcss, SiOpenai } from "react-icons/si";
...
const ProcessusDev = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {cards.map((card) => (
        <Card key={card.id} className="mb-6 dark:bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col">
          <CardHeader>
            <CardTitle>{card.title}</CardTitle>
            <CardDescription>{card.description}</CardDescription>
          </CardHeader>
          <CardContent className="flex-grow">
            <p className="text-gray-600 dark:text-gray-300 mb-4">{card.content}</p>
          </CardContent>
